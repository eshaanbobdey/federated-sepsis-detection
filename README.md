...
#### Architecture
- Input layer: number of features in the preprocessed PhysioNet feature set (exact count not documented here yet — add for reproducibility)
...
Evaluated on the [PhysioNet 2019 Challenge](https://physionet.org/content/challenge-2019/1.0.0/) dataset. *(Pending: exact dataset version/subset used, train/test split size, and split random seed — add here for full reproducibility.)*
...
- Document the exact dataset version/subset, train/test split size, and random seed used for the Results section above, for full reproducibility
